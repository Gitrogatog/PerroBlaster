    ...
    public static class Processor
    {
        ...
        public static void ProcessTexturePage(DirectoryInfo texturePageDir, DirectoryInfo textureOutputDir)
        {
            ...
            foreach (var directory in texturePageDir.EnumerateDirectories())
            {
                ...
                var getJSONfiles = directory.GetFiles("*.json");
                var animationMetadata = getJSONfiles.Length > 0 ?
                    JsonSerializer.Deserialize<CramTextureAtlasAnimationData>(File.ReadAllText(getJSONfiles[0].FullName), jsonSerializerOptions) :
                    default;
